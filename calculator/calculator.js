...
function insertNumbers() {
	// types number on click
	for (let button of numBtn) {
		button.addEventListener("click", (e) => {
			let input = e.target.textContent.trim();
			const total = calculatorScreen.textContent;

			if (total.length === 1 && total === "0")
				calculatorScreen.textContent = ""; // replaces initial 0 with clicked number
			if (total === "0" && input === "0") input = "0"; // not more then one 0 at the beginning
			if (total === "Cannot divide by zero!")
				calculatorScreen.textContent = ""; // prevents further inputs

			if (total.length > 24)
				calculatorScreen.style.padding = "30px 30px 70px 40px";
			if (total.length > 51) {
...
function insertOperators() {
	// types operator on click
	for (let button of operatorBtn) {
		button.addEventListener("click", function(e) {
			let input = e.target.textContent.trim();
			const total = calculatorScreen.textContent;
			const last = total[total.length - 1];
			const contains = e.target.classList.contains("operators"); //checks if the clicked operator contains the class .operators
			...
			if (total && !total[total.length - 1].includes(input)) {
				// doesn't allow two operators in a row
...
function calculations() {
	result.addEventListener("click", () => {
		...
		if (last === "0" && total[total.length - 2] === "/")
			calculatorScreen.textContent = "Cannot divide by zero!";
